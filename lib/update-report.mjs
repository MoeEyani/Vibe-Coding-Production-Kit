...
export function formatUpdatePlan(plan) {
  ...
  if (report.conflicts) lines.push('Resolve conflicts before applying the update. No project files were changed.');
  return lines.join('\n');
}
...
export async function checkForUpdate({ targetDir, fetchLatest = true }) {
  ...
  let latestVersion = cliVersion;
  let registryError = null;
  if (fetchLatest) {
    try { latestVersion = await fetchLatestVersion(); }
    catch (error) { latestVersion = null; registryError = error.message; }
  }
  const comparisonVersion = latestVersion ?? cliVersion;
  const updateAvailable = compareVersions(manifest.installedVersion, comparisonVersion) < 0;
  const cliUpdateAvailable = latestVersion ? compareVersions(cliVersion, latestVersion) < 0 : false;
  return {
    ...
    updateAvailable,
    cliUpdateAvailable,
    targetCommand: updateAvailable && latestVersion
