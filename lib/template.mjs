...
function policyForPath(relative) {
  ...
  if (relative === 'scripts/validate-framework.sh') return 'merge';
...
export async function buildDesiredFiles({ targetDir, agent = 'generic', stack = 'auto', includeGitHub = true }) {
  ...
  for (const relative of sourceFiles) {
    files.set(relative, {
      content: await readFile(path.join(packageRoot, relative), 'utf8'),
  ...
  for (const [relativeRaw, content] of adapterFiles(agent)) {
    ...
    files.set(relative, { content, mode: 0o644, policy: policyForPath(relative), origin: 'adapter' });
  }
  ...
  files.set('AGENTS.md', { ...agents, content: stackResult.content });
