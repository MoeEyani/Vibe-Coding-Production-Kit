...
export async function planUpdate({ targetDir, targetVersion = null, migrations = MIGRATIONS, desiredBuilder = buildDesiredFiles, currentVersion = null }) {
  ...
  for (const [relative, oldEntry] of Object.entries(oldFiles)) {
    ...
    const current = await readManagedFile(root, relative);
    const baseContent = await readBaseline(root, oldEntry);
    const explicitlyRemoved = removals.has(relative) || ![...renames.keys()].includes(relative);
    if (!explicitlyRemoved) continue;
    if (!current.exists) {
      actions.push(action('DETACH', relative, { reason: 'File was removed both locally and from the target VCP version.' }));
    } else if (oldEntry.policy === 'preserve' || current.content !== baseContent) {
      actions.push(action('DETACH', relative, { reason: 'File is no longer managed upstream; local content is preserved and detached from VCP management.' }));
    } else {
      actions.push(action('DELETE', relative, { reason: 'Unmodified VCP-managed file was removed upstream.' }));
    }
  }
