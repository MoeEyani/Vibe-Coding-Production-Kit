...
Usage:
  ...
  vibe-coding-production context <task> [--mode <name>] [--include <path>] [--planned <path>] [--output <path>]
  vcp context <task> [--mode <name>] [--include <path>] [--planned <path>] [--output <path>]
...
export async function runCli(args) {
  ...
  const result = await initProject({ targetDir: path.resolve(options.targetDir), agent: options.agent, stack: options.stack, includeGitHub: options.includeGitHub, force: options.force, dryRun: parsed.dryRun });
