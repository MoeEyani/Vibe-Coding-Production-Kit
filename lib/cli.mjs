...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') parsed.help = true;
      ...
      parsed.verifyOnly.push(value);
      index += 1;
    } else if (arg.startsWith('--only=')) {
      parsed.verifyOnly.push(arg.slice('--only='.length));
    } else if (arg === '--timeout-ms') {
      const value = args[index + 1];
      if (!value || value.startsWith('-')) throw new Error('--timeout-ms requires a value.');
      parsed.verifyTimeoutMs = Number(value);
      index += 1;
    } else if (arg.startsWith('--timeout-ms=')) {
      parsed.verifyTimeoutMs = Number(arg.slice('--timeout-ms='.length));
    } else if (arg === '--mode') {
      const value = args[index + 1];
