...
function safePath(root, candidate, label = 'path') {
  ...
  return { resolved, relative: path.relative(normalizedRoot, resolved) || '.' };
}
...
export function renderContextPack({ mode, task, prompt, agents, sources, extras }) {
  const sections = [
...
export async function createContextPack({
  ...
  const content = renderContextPack({ mode, task: taskFile, prompt, agents, sources: sourceFiles, extras: extraFiles });
