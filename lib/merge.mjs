...
export function threeWayMerge(baseText, currentText, nextText) {
  ...
  for (const left of currentHunks) {
    for (const right of nextHunks) {
      if (hunksConflict(left, right)) {
        return { clean: false, reason: `Overlapping edits around base lines ${Math.min(left.start, right.start) + 1}-${Math.max(left.end, right.end) + 1}.` };
      }
    }
  }
  ...
  for (const hunk of nextHunks) {
    const duplicate = all.find((candidate) => candidate.start === hunk.start && candidate.end === hunk.end && arraysEqual(candidate.lines, hunk.lines));
  ...
  const finalNewline = currentSplit.hasFinalNewline || nextSplit.hasFinalNewline || baseSplit.hasFinalNewline;
