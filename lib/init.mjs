...
import { applyStackProfile, detectStack } from './stacks.mjs';
...
export async function initProject({ targetDir, agent = 'generic', stack = 'auto', includeGitHub = true, force = false, dryRun = false }) {
  ...
  if (dryRun) {
    return { target, files: planned, note: agentNote(agent), stack: stack === 'auto' ? await detectStack(target) : stack, dryRun: true };
  }
  ...
  const stackResult = await applyStackProfile(target, stack);
  if (stackResult.changed) {
    await writeFile(path.join(target, 'AGENTS.md'), stackResult.content, 'utf8');
  }
  ...
  return { target, files: planned, note: agentNote(agent), stack: stackResult.stack, dryRun: false };
}
