...
function templateSignalsFor(relative) {
  return SOURCE_TEMPLATE_SIGNALS[relative] ?? [];
}
...
async function sourceTruthChecks(root, content) {
  ...
  if (refs.length === 0) {
    return { valid: 0, refs: [], issues: ['No repository-local source-of-truth reference is defined.'], templateRefs: [] };
  }
  ...
  for (const ref of refs) {
    ...
    if (signals.length > 0) {
      ...
      if (signals.some((signal) => referenced.includes(signal))) templateRefs.push(safe.relative);
    }
  }

  return { valid, refs, issues, templateRefs: [...new Set(templateRefs)] };
}
